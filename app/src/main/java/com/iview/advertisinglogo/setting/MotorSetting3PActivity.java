...
public class MotorSetting3PActivity extends Activity {
    private final static String TAG = "MotorSetting3PActivity";
    ...
    public enum SETTING_STATE {
        INIT, POINT_A, POINT_C, POINT_D, END
    }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        switch (keyCode) {
            case KeyEvent.KEYCODE_DPAD_CENTER:
                ...
                } else if (state == SETTING_STATE.POINT_C) {
                    int hsteps = commonManager.getMotorSteps(CommonManager.HMotor);
                    int vsteps = commonManager.getMotorSteps(CommonManager.VMotor);
                    Log.e(TAG, "pointA h:" + pointA[0] + " , v:" + pointA[1]);
                    Log.e(TAG, "set h:" + pointA[0] + " , v:" + pointA[1]);

                    if (Math.abs(vsteps - pointA[1]) < 300 && Math.abs(hsteps - pointA[0]) < 300) {
                        Log.e(TAG, "vstep can't less than 100");
                ...
                } else if (state == SETTING_STATE.POINT_D) {
                    int hsteps = commonManager.getMotorSteps(CommonManager.HMotor);
                    int vsteps = commonManager.getMotorSteps(CommonManager.VMotor);

                    if (Math.abs(vsteps - pointC[1]) < 300 && Math.abs(hsteps - pointC[0]) < 300) {
                        Log.e(TAG, "vstep can't less than 100");
                        Toast.makeText(this, "两个巡航点不能相等", Toast.LENGTH_LONG).show();
                        break;
                    } else {
                        ...
                        setPointA(pointA[0], pointA[1]);
                        setPointC(pointC[0], pointC[1]);
                        setPointD(pointD[0], pointD[1]);
...
//                break;
    ...
    public void setPointC(int hsteps, int vsteps) {
        SharedPreferences sharedPreferences= getSharedPreferences("motorData", Context.MODE_PRIVATE);
        //步骤2： 实例化SharedPreferences.Editor对象
        SharedPreferences.Editor editor = sharedPreferences.edit();

        editor.putInt("pointCHstep", hsteps);
        editor.putInt("pointCVstep",  vsteps);

        editor.commit();
    }

    public void setPointD(int hsteps, int vsteps) {
        SharedPreferences sharedPreferences= getSharedPreferences("motorData", Context.MODE_PRIVATE);
        //步骤2： 实例化SharedPreferences.Editor对象
        SharedPreferences.Editor editor = sharedPreferences.edit();

        editor.putInt("pointDHstep", hsteps);
        editor.putInt("pointDVstep",  vsteps);
